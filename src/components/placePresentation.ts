export const placeTypeLabels = {
  destination: "Destination",
  hotel: "Hotel",
  activity: "Activity",
  attraction: "Attraction",
  restaurant: "Restaurant",
  shopping: "Shopping",
};

export const placeTypeIcons = {
  destination: `<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M12 2c-3.31 0-6 2.69-6 6 0 4.5 6 12 6 12s6-7.5 6-12c0-3.31-2.69-6-6-6zm0 8.5A2.5 2.5 0 1 1 12 5.5a2.5 2.5 0 0 1 0 5z"/>
  </svg>`,
  hotel: `<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M4 7h10a4 4 0 0 1 4 4v6h-2v-2H6v2H4V7zm2 6h10v-2a2 2 0 0 0-2-2H6v4zm1-3h3v2H7v-2zm12 5h-1V7h1a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1z"/>
  </svg>`,
  activity: `<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M13 3a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm-2.4 4.1 2.5 1.2 2.2 2.2-1.4 1.4-1.8-1.8-1.3-.6-.7 2.6 2.4 2.2V21h-2v-5.8l-2.3-2.1-.8 3.1-2.9-.8.5-1.9 1 .3 1.5-5.5a2 2 0 0 1 2.6-1.2zM17 12h2v9h-2v-9z"/>
  </svg>`,
  attraction: `<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M4 20h16v-2H4v2zm2-4h2V8H6v8zm4 0h2V8h-2v8zm4 0h2V8h-2v8zm4 0h2V8h-2v8zM3 6l9-4 9 4v2H3V6z"/>
  </svg>`,
  restaurant: `<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M6 2a1 1 0 0 1 1 1v7a3 3 0 0 1-2 2.82V22H3v-9.18A3 3 0 0 1 1 10V3a1 1 0 1 1 2 0v7a1 1 0 1 0 2 0V3a1 1 0 0 1 1-1zm8 0a3 3 0 0 1 3 3v7h-2v10h-2V12h-2V5a3 3 0 0 1 3-3z"/>
  </svg>`,
  shopping: `<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M7 7a5 5 0 0 1 10 0h3a1 1 0 0 1 1 1l-1 12a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2L3 8a1 1 0 0 1 1-1h3zm2 0h6a3 3 0 0 0-6 0zm0 4a1 1 0 0 1 1 1 2 2 0 0 0 4 0 1 1 0 1 1 2 0 4 4 0 0 1-8 0 1 1 0 0 1 1-1z"/>
  </svg>`,
};

export const placeStatusLabels = {
  included: "Included",
  possible: "Possible",
  avoid: "Avoid",
};

export const placeStatusChipLabels = {
  possible: "Possible",
  avoid: "Avoid",
};

export const placeStatusColors = {
  included: "var(--status-included)",
  possible: "var(--status-possible)",
  avoid: "var(--status-avoid)",
};
